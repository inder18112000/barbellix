...
export interface WorkoutPlanEditorSubmitInput {
  name: string
  goal: FitnessGoal
  days: Array<{ dayLabel: string; exercises: Array<{ exerciseId: string; sets: number; reps: string; restSecs: number }> }>
  changeNote?: string
}

export interface WorkoutPlanEditorInitialDay {
  dayLabel: string
  exercises: Array<{ exerciseId: string; name: string; sets: number; reps: string; restSecs: number }>
}

interface Props {
  initialName?: string
  initialGoal?: FitnessGoal
  initialDays?: WorkoutPlanEditorInitialDay[]
  onSubmit: (input: WorkoutPlanEditorSubmitInput) => void
  isPending: boolean
  submitLabel: string
  /** Shows a "What changed?" note field above the submit button - used when editing an existing
   * member's plan (feeds the plan's changeSummary, already rendered elsewhere in the admin UI),
   * not shown when creating a brand new template. */
  showChangeNote?: boolean
}

/** Shared by admin's CreateWorkoutPlanPage and EditMemberPlanPage - the trainer-side
 * CreatePlanPage.tsx is left as its own copy (already working, low risk to leave untouched)
 * rather than migrated to this component. */
export function WorkoutPlanEditor({ initialName, initialGoal, initialDays, onSubmit, isPending, submitLabel, showChangeNote }: Props) {
  const [name, setName] = useState(initialName ?? '')
  const [goal, setGoal] = useState<FitnessGoal>(initialGoal ?? 'general_fitness')
  ...
  const [days, setDays] = useState<DayState[]>(() =>
    initialDays && initialDays.length > 0
      ...
      : [{ key: nextKey(), dayLabel: 'Day 1', exercises: [] }],
  )
  ...
  const handleSubmit = () => {
    onSubmit({
      ...
      changeNote: showChangeNote ? changeNote || undefined : undefined,
    })
  }
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-4">
      ...
      {days.map((day, idx) => (
        <Card key={day.key}>
          <CardHeader className="flex-row items-center justify-between">
            <Input value={day.dayLabel} onChange={(e) => updateDayLabel(day.key, e.target.value)} className="max-w-56 font-medium" />
            {days.length > 1 && (
      ...
      {showChangeNote && (
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="changeNote">What changed? (optional)</Label>
          <Input id="changeNote" value={changeNote} onChange={(e) => setChangeNote(e.target.value)} placeholder="e.g. Reduced volume, swapped in dumbbell rows" />
        </div>
      )}

      <Button className="w-fit" disabled={!canSubmit || isPending} onClick={handleSubmit}>
